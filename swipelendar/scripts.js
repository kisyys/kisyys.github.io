...
function dayview(number, number2) {
    ...
    topic.insertAdjacentHTML("beforeend", `<div class="topic";> ${ "<b>" +  dayName2 + " - Test26" +  "</b>"}</div>`);
    ...
    if(old>0) {
        ...
        for(let hour = 1; hour <= 24; hour++) {
            ...
            if(now+1==hour && number==today && month == d2.getMonth()+1 && year == d2.getFullYear()) {
                ...
                x = hour;
                history2.push(` <div class="now"${` id="divhour` + hour + `";>` + `<p style="color:black";>` + "<b>"+ previous + ":00" + "-" + hour + ":00" + "  " +  "<br>" + "Now" + "</b>" + "</p>"}</div>`);
            }
