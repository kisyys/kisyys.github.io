...
if(!localStorage.getItem("day_history2"))  {
    var history2 = [];
} else {
    var history2 = localStorage.getItem("day_history2");
}
...
function dayview(number, number2) {
    ...
    topic.insertAdjacentHTML("beforeend", `<div class="topic";> ${ "<b>" +  dayName2 + " - Test16" +  "</b>"}</div>`);
    ...
    if(old>0) {
        ...
        for(let hour = 1; hour <= 24; hour++) {
            ...
            if(now+1==hour && number==today && month == d2.getMonth()+1 && year == d2.getFullYear()) {
                ...
                x = hour;
                history2.push(`"beforeend", <div class="now"${` id="divhour` + hour + `";>` + `<p style="color:black";>` + "<b>"+ previous + ":00" + "-" + hour + ":00" + "  " +  "<br>" + "Now" + "</b>" + "</p>"}</div>`);
            }
    ...
    function done(numberx) {
        ...
        history2[old2+t3[0]] = `<p style="color:black";>` + t2[0] + "  " + "</p>" +  "<del>" + t2[1] + "</del>"; 
