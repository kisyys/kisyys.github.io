...
function stats() {   
    ...
    if(today_score/today_possible<0.5 || today_score==0) {
        status.insertAdjacentHTML("beforeend",  "<b> Status: LOSER! DO MORE!</b> <br>");
    }
    else if(today_score/today_possible==1) {
        status.insertAdjacentHTML("beforeend",  "<b> Status: YOU ARE A WINNER!</b> <br> " +  `<img src="makeit2.gif" alt="">` );
    }
    else {
        status.insertAdjacentHTML("beforeend",  "<b> Status: GOOD PROGRESS, YOU CAN DO IT!</b> <br>");
    }
...
function tasks(){  
	...
	<br>
	<br>
    <label for="stask2">Special task:</label>
...
function dayview(number, number2) {  
    ...
    topic.insertAdjacentHTML("beforeend", `<div class="topic";> ${ "<b>" +  dayName2 + " - Test36" +  "</b>"}</div>`);
    ...
    // doit.insertAdjacentHTML("beforeend", `<p id="p11" style="text-align:left" onclick="done(0)"> <b> Do it </b> </p>`); 
    ...
    function handleTouchMove0(evt) {
        ...
        if ( Math.abs( xDiff ) > Math.abs( yDiff ) ) {/*most significant*/
            if ( xDiff > 0 ) {
                ...
                if(display == 1 && content.includes("<del>") && number==today && month == d2.getmonth()+1 && year == d2.getfullyear())  {                
                    undone(numb);
                }
...
function back(number) {
    ...
    const tasks = document.querySelector("#p1")
    const next = document.querySelector("#p2");
    const stats = document.querySelector("#p11");

    previous.insertAdjacentHTML("beforeend", `<p id="p0" style="text-align:left" onclick="step(0)"> <b> Previous </b> </p>`);
    stats.insertAdjacentHTML("beforeend", `<p id="p11" style="text-align:center" onclick="stats()"> <b> Stats </b> </p>`);
    tasks.insertAdjacentHTML("beforeend", `<p id="p1" style="text-align:center" onclick="tasks()"> <b> Tasks </b> </p>`);
